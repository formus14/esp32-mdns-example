...
static void initialise_mdns(void)
{
    ...
    ESP_ERROR_CHECK( mdns_hostname_set("_ekey_test") );
    //set default mDNS instance name
    ESP_ERROR_CHECK( mdns_instance_name_set("_http") );
	
	ESP_ERROR_CHECK( mdns_service_add("ESP32-ekey", "_http", "_tcp", 5353 , NULL, 0) );

    mdns_service_instance_name_set("_http", "_tcp", "ESP32-ekey");
...
static void mdns_example_task(void *pvParameters)
{
    ...
    xEventGroupWaitBits(wifi_event_group, IP4_CONNECTED_BIT | IP6_CONNECTED_BIT,
		...
		struct ip4_addr addr;
		addr.addr = 0;
		esp_err_t err = mdns_query_a("_ekey_test", 2000,  &addr);
	while(1) {
